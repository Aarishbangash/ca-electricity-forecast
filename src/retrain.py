...
def update_dataset(api_key: str) -> bool:
    """
    Fetch yesterday's data and append to dataset.
    Retries 3 times. Falls back to day before if needed.
    ...
    if eia_df is None:
        ...
        print(f"  Trying fallback: {day_before}")
    ...
    if eia_df is None:
        print("  ERROR: EIA unavailable — retraining on existing data")
        return True   # Continue retrain with existing data

    # Fetch weather
    weather_df = fetch_weather_archive(yesterday)
    if weather_df is None:
        print("  ERROR: Weather unavailable — retraining on existing data")
        return True   # Continue retrain with existing data
    ...
    cutoff   = pd.Timestamp.now(tz=TIMEZONE) - pd.DateOffset(years=5)
    combined = combined[
...
def prepare_data():
    """Load CSV, build features, split train/val."""
...
def save_results(model, val_mape, df_train, df_val):
    ...
    print(f"  Data to  : {metrics['data_to']}")
...
def run_retrain():
    ...
    update_dataset(api_key)      # never aborts even if API down
    df_train, df_val, FEATURE_COLS = prepare_data()
    model, val_mape = retrain_xgboost(df_train, df_val, FEATURE_COLS)
